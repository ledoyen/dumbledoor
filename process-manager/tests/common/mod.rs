...
use process_manager::{ProcessConfig, ProcessStatus};
...
pub fn get_cleanup_timeout() -> std::time::Duration {
    #[cfg(target_os = "windows")]
    {
...
pub fn get_processes_by_pattern(pattern: &str) -> Vec<u32> {
    ...
    #[cfg(unix)]
    {
        let output = std::process::Command::new("pgrep").arg(pattern).output();
...
mod tests {
    ...
    fn test_minimal_environment() {
        ...
        #[cfg(windows)]
        {
            ...
            let _ = env.len(); // Validates env is accessible
        }
    }
}
